...
| `STRANDS_DASHBOARD_PEER_TTL_S` | Dashboard mesh bridge: seconds a peer may stay silent before it is dropped from the fleet snapshot entirely (a card greys out at 15 s regardless). A non-positive value never prunes. A value that is not a finite number (`nan`, `inf`, a typo) is refused with a log line and the default is used | `300` |
| `STRANDS_DASHBOARD_PRESENCE_HZ` | Dashboard mesh bridge: ceiling on UNCHANGED `presence` repeats forwarded to the browser, in Hz. A changed reading is never delayed; `0` removes the ceiling. Same value domain as `STRANDS_DASHBOARD_PEER_TTL_S` | `1.0` |
| `STRANDS_DASHBOARD_CAMERA_META_HZ` | Dashboard mesh bridge: ceiling on unchanged `camera_meta` repeats, in Hz | `2.0` |
| `STRANDS_DASHBOARD_POSE_HZ` | Dashboard mesh bridge: ceiling on unchanged `pose` repeats, in Hz | `1.0` |
| `STRANDS_DASHBOARD_IMU_HZ` | Dashboard mesh bridge: ceiling on unchanged `imu` repeats, in Hz | `1.0` |
| `STRANDS_DASHBOARD_ODOM_HZ` | Dashboard mesh bridge: ceiling on unchanged `odom` repeats, in Hz | `1.0` |
| `STRANDS_DASHBOARD_LIDAR_HZ` | Dashboard mesh bridge: ceiling on unchanged `lidar` summary repeats, in Hz | `1.0` |
| `STRANDS_DASHBOARD_HEALTH_HZ` | Dashboard mesh bridge: ceiling on unchanged `health` repeats, in Hz; matches the 0.5 Hz the mesh publishes health at | `0.5` |
